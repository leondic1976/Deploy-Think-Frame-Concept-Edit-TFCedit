...
- 밝게·어둡게·시스템 테마
- 인터페이스·문서 글꼴과 크기, 줄 간격, 읽기 좋은 본문 폭, 맞춤법 검사 설정
- Gemini·Groq·OpenRouter 무료 한도와 로컬 Ollama 빠른 설정
...
설정의 **파일과 저장 > 현재 작업폴더**에서 다른 로컬 폴더로 변경할 수 있습니다. 설치·업데이트 과정은 설치 폴더의 `workspaces`를 애플리케이션 파일과 구분해 보존합니다.

## 화면과 편집기 설정

설정의 **화면 모양**에서 프로그램 인터페이스와 문서 본문을 따로 조절합니다.

- 인터페이스 글꼴과 크기
- 문서 글꼴과 12~36px 본문 크기
- 좁게·보통·넓게 줄 간격
- 긴 화면에서 본문 폭을 제한하는 읽기 좋은 줄 너비
- 운영체제 맞춤법 검사

이전 버전의 작은 기본값은 처음 불러올 때 읽기 쉬운 18px 문서 크기로 자동 보정됩니다.

## AI 연결

설정에서 OpenAI 호환 API 및 Claude, Gemini, Groq, Grok, Meta AI, OpenRouter, Ollama, DeepSeek, Qwen, Moonshot, 豆包(두바오) 등 다중 제공자를 선택해 사용할 수 있습니다.

무료로 시험할 수 있는 빠른 선택지는 다음과 같습니다. 무료 한도와 모델은 각 제공자의 정책에 따라 변경될 수 있습니다.

- Gemini 무료 등급: Google AI Studio API 키 사용
- Groq 무료 개발자 플랜
- OpenRouter의 `openrouter/free` 무료 모델 라우터
- Ollama: API 키 없이 내 컴퓨터에서 로컬 모델 실행

- Base URL: 선택한 제공자의 공식 기본 URL
- API Key: 제공자별로 분리해 운영체제 보안 저장소에 암호화
- Model: 사용할 모델 이름
- 요청 제한 시간
- 연결 테스트
