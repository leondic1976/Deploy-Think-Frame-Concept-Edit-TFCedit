# 변경 기록

## [Unreleased]

### 추가

- 설치된 ThinkFrame 실제 화면과 단계별 사용법을 제공하는 소개 페이지

## [0.2.2] - 2026-07-25

### 추가

- 최신 릴리스와 운영체제별 다운로드를 제공하는 GitHub Pages 사이트

### 변경

- 커스텀 창 제어 버튼을 제거하고 운영체제 기본 창 프레임 사용
- 문서 상태와 패널·설정 제어를 단순한 앱 도구 모음으로 분리

### 수정

- GitHub Release 게시 스크립트의 셸 조건문 종료
- 성공한 빌드 산출물을 다시 게시하는 수동 릴리스 복구 작업
- 체크아웃 없는 게시 작업에 GitHub 저장소 식별자 전달
- Windows PowerShell 5.1과 호환되는 설치 임시 폴더 생성

## [0.2.1] - 2026-07-25

### 수정

- Linux DEB 제작기가 대소문자를 구분하는 실행 파일 이름을 사용하도록 수정

## [0.2.0] - 2026-07-25

### 추가

- macOS Apple Silicon·Intel DMG와 ZIP 패키지
- Debian·Ubuntu Linux x64 DEB와 ZIP 패키지
- macOS·Linux용 체크섬 검증 터미널 설치 스크립트
- 운영체제별 네이티브 GitHub Actions 빌드와 통합 릴리스

### 수정

- 여러 플랫폼의 통합 체크섬에서 Windows 설치 파일을 정확히 검증
- Windows 알림과 바로가기를 위한 앱 사용자 모델 ID
- 제한된 Windows 실행 정책에서도 동작하는 PowerShell 설치 명령

### 문서

- 현재 버전, 운영체제별 직접 다운로드 경로, 업데이트 구조

## [0.1.1] - 2026-07-25

### 수정

- Windows CI에서 임시 경로의 짧은 이름과 실제 경로가 달라지는 테스트 문제

## [0.1.0] - 2026-07-25

### 추가

- Markdown·TXT 로컬 작업공간과 일반 텍스트 편집
- 문서 내 찾기·바꾸기와 행·열 표시
- 안전한 저장, 자동 저장, 외부 변경 충돌 감지
- 문서 이름 변경, 다른 이름으로 저장, 복제, 휴지통 이동
- 체계화·구조화·개념검증·현실화·최종 프롬프트 AI 흐름
- 운영체제 암호화를 사용하는 API 키 저장
- 밝게·어둡게·시스템 테마와 키보드 단축키
- Windows x64 Squirrel 설치 파일
- SHA-256을 검증하는 PowerShell 설치 스크립트
- Git 태그 기반 GitHub Release 자동 배포

[Unreleased]: https://github.com/leondic1976/Deploy-Think-Frame-Concept-Edit-TFCedit/compare/v0.2.2...HEAD
[0.2.2]: https://github.com/leondic1976/Deploy-Think-Frame-Concept-Edit-TFCedit/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/leondic1976/Deploy-Think-Frame-Concept-Edit-TFCedit/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/leondic1976/Deploy-Think-Frame-Concept-Edit-TFCedit/compare/v0.1.1...v0.2.0
[0.1.1]: https://github.com/leondic1976/Deploy-Think-Frame-Concept-Edit-TFCedit/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/leondic1976/Deploy-Think-Frame-Concept-Edit-TFCedit/releases/tag/v0.1.0
